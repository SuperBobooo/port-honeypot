...
- `log_max_bytes` / `log_backup_count`：服务端 `logs/server.log` 的轮转大小与保留份数。
- `alerts`：告警通道和限频。
...
客户端日志默认超过 2MB 后轮转，保留 5 份备份；可在 `client_config.json` 中通过 `log_max_bytes` 和 `log_backup_count` 调整。
...
```json
"alerts": {
  "event_types": ["attack", "node_disconnect", "abnormal_probe"],
  "abnormal_probe_window_seconds": 120,
  "abnormal_probe_distinct_ports": 4,
  "abnormal_probe_min_events": 6
}
```

含义：同一源 IP 在 120 秒内产生至少 6 次访问且涉及至少 4 个不同目标端口时，触发 `abnormal_probe` 告警。该告警同样受 `rate_limit_seconds` 限频保护。
...
- 编辑并保存 `client_config.json`。
- 启动 / 停止客户端进程。
- 查看 `status` 命令输出和客户端本地日志。
- 安装 / 卸载客户端开机自启动。
- 系统托盘常驻，支持打开 / 隐藏主界面和退出二次确认。
